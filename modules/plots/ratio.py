...
import datetime
from mytypes.indicators import RatioIndicator, RatioIndicatorColumns
...
def m_create_ratio_df(indicators, tickers, price):
    ...
    if len(indicators) == 1 and indicators[0] == "all":
        indicators = [i for i in RatioIndicator if i != "all"]
    ...
    with Progress(SpinnerColumn(),
        ...
        for ticker in tickers:
            all_indicators = ' '.join([f'{RatioIndicatorColumns[i]} as "{ticker}_{RatioIndicatorColumns[i]}",' for i in indicators])
            only_aliases = [f"{ticker}_{RatioIndicatorColumns[i]}" for i in indicators]
            dfquery = sqlio.read_sql_query(f"""
                SELECT DISTINCT period_end as reference_date, {all_indicators} 0 as "{ticker}" FROM ratios mr
